...
public class CatClassLoader extends ClassLoader {
	...
	public CatClassLoader(String name) {
		super(); // 通过这个构造方法生成的类加载器，它的父加载器是系统类加载器
		this.name = name;
	}

	public CatClassLoader(String name, ClassLoader loader) {
	...
	protected Class<?> findClass(String name) throws ClassNotFoundException {
		...
		try {
			fis = new FileInputStream("D:\\workspace_study\\study\\target\\classes\\study\\classload\\custom\\" + name + ".class");
		...
		return this.defineClass("study.classload.custom." + name, data, 0, data.length, null);
	}
}